...
class Utility(object):
    ...
    @classmethod
    def assemble_response_from_log_message(
        cls,
        response,
        ...
        if response_status_code in (requests.codes.OK, requests.codes.FOUND):
        ...
        return (
            ...
            '[Status Code]\n'
            '=============\n{2}\n'.format(
                response.request.method, response.url, response_status_code
            )
        )
    ...
    def determine_ip_address_location(cls):
        ...
        try:
            response = requests.get(
                'http://ip-api.com/json', headers={'Accept': 'application/json'}
            )
    ...
    def determine_ip_address_type(cls, ip_address):
        ...
        if ip_address_object.is_loopback:
            ...
            ip_address_object in ipaddress.ip_network('10.0.0.0/8')
            or ip_address_object in ipaddress.ip_network('172.16.0.0/12')
            or ip_address_object in ipaddress.ip_network('192.168.0.0/16')
        ):
    ...
    def is_valid_private_hostname(cls, hostname_private):
        ...
        try:
            ...
            if (
                ip_address_object not in ipaddress.ip_network('10.0.0.0/8')
                and ip_address_object not in ipaddress.ip_network('172.16.0.0/12')
                and ip_address_object not in ipaddress.ip_network('192.168.0.0/16')
            ):
                is_valid_private_hostname = False
        except ValueError:
            # This is a weak attempt to differentiate between a badly
            # input IP address and a hostname.
            if re.match(
                r'\A\d+\.\d+.\d+.\d+\Z', hostname_private
            ) or not Utility.is_valid_hostname(hostname_private):
                is_valid_private_hostname = False
    ...
    def is_valid_public_hostname(cls, hostname_public):
        ...
        try:
            ...
            # input IP address and a hostname.
            if re.match(
                r'\A\d+\.\d+.\d+.\d+\Z', hostname_public
            ) or not Utility.is_valid_hostname(hostname_public):
                is_valid_public_hostname = False
    ...
    def read_file(cls, file_path, in_binary=False, in_base_64=False):
        try:
            with open(file_path, 'r{0}'.format('b' if in_binary else '')) as input_file:
                file_content = (
                    base64.b64encode(input_file.read())
                    if in_base_64
                    else input_file.read()
                )
    ...
    def write_file(cls, file_content, file_path, in_binary=False):
        try:
            with open(
                file_path, 'w{0}'.format('b' if in_binary else '')
            ) as output_file:
