...
from iptv_proxy.providers.smoothstreams.db import Base

logger = logging.getLogger(__name__)


class SmoothStreamsChannel(Base):
    ...
    __table_args__ = (Index('{0}_channel_ix_id'.format(_provider_name), _id.asc()),
                      ...
                      Index('{0}_channel_ix_m3u8_group_number'.format(_provider_name),
                            _m3u8_group.asc(),
                            _number.asc()),
...
class SmoothStreamsProgram(Base):
    _provider_name = SmoothStreamsConstants.PROVIDER_NAME.lower()
    ...
    __table_args__ = (
        ...
        Index('{0}_program_ix_channel_number_start'.format(_provider_name), _channel_number.asc(), _start.asc()),
        Index('{0}_program_ix_channel_xmltv_id_start'.format(_provider_name), _channel_xmltv_id.asc(), _start.asc()),
        Index('{0}_program_ix_channel_xmltv_id_start_stop'.format(_provider_name),
...
class SmoothStreamsSetting(Base):
    _provider_name = SmoothStreamsConstants.PROVIDER_NAME.lower()
