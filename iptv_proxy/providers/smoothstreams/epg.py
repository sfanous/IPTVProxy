...
class SmoothStreamsEPG(ProviderEPG):
            ...
            for (event, element) in etree.iterparse(epg_xml_stream,
                ...
                if event == 'end':
                    if element.tag == 'channel':
                        ...
                        db_session.add(SmoothStreamsProgram(
        ...
        try:
            ...
            for (prefix, event, value) in ijson_parser:
                if (prefix, event) == ('generated_on', 'string'):
                    ...
                    programs.append(XMLTVProgram(provider='SmoothStreams',
                                                 start=program_start,
                                                 stop=program_stop,
                                                 pdc_start=None,
                                                 vps_start=None,
                                                 ...
                                                 channel_xmltv_id=None,
                                                 clump_index=None,
                                                 titles=[XMLTVTitle(language=None,
                                                                    text=program_title)],
                                                 sub_titles=[],
                                                 descriptions=[XMLTVDescription(language=None,
    ...
    def _request_fog_channels_json(cls):
        url = '{0}{1}'.format(SmoothStreamsConstants.FOG_EPG_BASE_URL,
                              SmoothStreamsConstants.FOG_CHANNELS_JSON_FILE_NAME)

        logger.debug('Downloading {0}\n'
                     'URL => {1}'.format(SmoothStreamsConstants.FOG_CHANNELS_JSON_FILE_NAME, url))
    ...
    def _request_fog_epg_xml(cls):
        ...
        logger.debug('Downloading {0}\n'
                     'URL => {1}'.format(SmoothStreamsConstants.FOG_EPG_XML_FILE_NAME, url))
    ...
    def _request_smoothstreams_epg_json(cls):
        ...
        logger.debug('Downloading {0}\n'
                     'URL => {1}'.format(SmoothStreamsConstants.EPG_FILE_NAME, url))
