...
class CoolAsIce(SmartersProvider):
    __slots__ = []
    ...
    def _generate_playlist_m3u8_static_track_url(cls, track_information, **kwargs):
        ...
        username = Configuration.get_configuration_parameter('COOLASICE_USERNAME')
        password = SecurityManager.decrypt_password(
            Configuration.get_configuration_parameter('COOLASICE_PASSWORD')).decode()
    ...
    def _initialize_class_variables(cls):
        try:
            cls.set_do_reduce_hls_stream_delay(
                OptionalSettings.get_optional_settings_parameter('reduce_coolasice_delay'))
        except KeyError:
            pass
