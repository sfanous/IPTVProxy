...
class SmoothStreams(IPTVProxyProvider):
    __slots__ = []
    ...
    def _refresh_session(cls):
        ...
        if IPTVProxyConfiguration.get_configuration_parameter('SMOOTH_STREAMS_SERVICE') == 'viewmmasr':
            url = 'https://www.mma-tv.net/loginForm.php'
        else:
            url = 'https://auth.smoothstreams.tv/hash_api.php'

        username = IPTVProxyConfiguration.get_configuration_parameter('SMOOTH_STREAMS_USERNAME')
        password = IPTVProxySecurityManager.decrypt_password(
            IPTVProxyConfiguration.get_configuration_parameter('SMOOTH_STREAMS_PASSWORD')).decode()
        site = IPTVProxyConfiguration.get_configuration_parameter('SMOOTH_STREAMS_SERVICE')
    ...
    def download_chunks_m3u8(cls, client_ip_address, client_uuid, requested_path, requested_query_string_parameters):
        ...
        http_session = cls._get_session_parameter('http_session')
    ...
    def generate_playlist_m3u8_tracks(cls, generate_playlist_m3u8_tracks_mapping):
        ...
        if playlist_protocol not in VALID_SMOOTH_STREAMS_PLAYLIST_PROTOCOL_VALUES:
            playlist_protocol = IPTVProxyConfiguration.get_configuration_parameter('SMOOTH_STREAMS_PLAYLIST_PROTOCOL')

        if playlist_type not in VALID_SMOOTH_STREAMS_PLAYLIST_TYPE_VALUES:
            playlist_type = IPTVProxyConfiguration.get_configuration_parameter('SMOOTH_STREAMS_PLAYLIST_TYPE')
        ...
        for channel_record in channel_records:
            ...
            if playlist_type == 'dynamic':
                ...
                tracks.append('{0}://{1}.smoothstreams.tv:{2}/{3}/ch{4:02}q1.stream?wmsAuthSign={5}\n'.format(
                    'https' if playlist_protocol == 'hls' else 'rtmp',
                    IPTVProxyConfiguration.get_configuration_parameter('SMOOTH_STREAMS_SERVER'),
    ...
    def get_supported_protocols(cls):
        return VALID_SMOOTH_STREAMS_PLAYLIST_PROTOCOL_VALUES

    @classmethod
    def initialize(cls):
        password_in_configuration_file = IPTVProxyConfiguration.get_configuration_parameter('SMOOTH_STREAMS_PASSWORD')
