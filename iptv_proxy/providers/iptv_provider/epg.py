...
class ProviderEPG(ABC):
    ...
    @classmethod
    def _apply_channel_transformations(
        cls,
        channel,
        channel_name_map,
        ...
        if channel_group_map is not None:
            for channel_group_map_regular_expression in channel_group_map['name']:
                for display_name in channel.display_names:
                    if re.search(
                        channel_group_map_regular_expression, display_name.text
                    ):
                        channel.m3u8_group = channel_group_map['name'][
                            channel_group_map_regular_expression
                        ]
        ...
        if m3u8_group_map is not None:
            for m3u8_group_map_regular_expression in m3u8_group_map:
                if re.search(m3u8_group_map_regular_expression, channel.m3u8_group):
                    channel.m3u8_group = m3u8_group_map[
                        m3u8_group_map_regular_expression
                    ]

                    break

        channel.xmltv_id = '{0}'.format(
            uuid.uuid3(
                uuid.NAMESPACE_OID,
                '{0} - {1}'.format(channel.number, channel.m3u8_group),
            )
        )
        ...
        if do_use_iptv_proxy_icons:
            for channel_icon_file_name in os.listdir(
                os.path.join(CHANNEL_ICONS_DIRECTORY_PATH, cls._provider_name)
            ):
    ...
    def _calculate_epg_settings_md5(cls, **kwargs):
        ...
        for kwarg in sorted(kwargs):
            if (
                hasattr(cls, '_{0}'.format(kwarg))
                and getattr(cls, '_{0}'.format(kwarg)) is not None
            ):
        ...
        if (
            epg_settings['epg_source']
            == ProvidersController.get_provider_map_class(cls._provider_name)
            .epg_source_enum()
            .OTHER.value
        ):
        ...
        return hashlib.md5(
            json.dumps(epg_settings, sort_keys=True).encode()
        ).hexdigest()
    ...
    def _do_update_epg(cls, **kwargs):
        ...
        with provider_map_class.database_class().get_access_lock().shared_lock:
            ...
            try:
                ...
                for (
                    setting_row
                ) in provider_map_class.database_access_class().query_settings(
                    db_session
                ):
                    if setting_row.name == 'epg_settings_md5':
                        ...
                        last_epg_refresh_date_time_in_utc = datetime.strptime(
                            setting_row.value, '%Y-%m-%d %H:%M:%S%z'
                        )

                current_date_time_in_utc = datetime.now(pytz.utc)

                if cls._calculate_epg_settings_md5(
                    **kwargs
                ) != epg_settings_md5 or current_date_time_in_utc >= last_epg_refresh_date_time_in_utc + timedelta(
                    days=1
                ):
                    do_update_epg = True
                else:
                    with cls._update_times_lock.reader_lock:
                        ...
                        for update_time in cls._update_times:
                            next_epg_refresh_date_time_in_utc = (
    ...
    def _initialize_refresh_epg_timer(cls, do_set_timer_for_retry=False):
        ...
        if do_set_timer_for_retry:
            refresh_epg_date_time_in_utc = (
                current_date_time_in_utc.astimezone(tzlocal.get_localzone()).replace(
            ...
            cls._start_refresh_epg_timer(
                (
                    refresh_epg_date_time_in_utc - current_date_time_in_utc
                ).total_seconds()
            )
        else:
            with cls._update_times_lock.reader_lock:
    ...
    def _parse_external_epg_xml(cls, db_session, **kwargs):
        ...
        channel_name_map = kwargs.get('channel_name_map', {})
        do_use_provider_icons = kwargs.get('do_use_provider_icons', True)
            ...
            for (event, element) in etree.iterparse(
                ...
                if event == 'end':
                    if element.tag == 'channel':
                        channel_m3u8_group = (
                            provider_map_class.constants_class().PROVIDER_NAME
                        )
                        channel_number = None
                        ...
                        for sub_element in list(element):
                            if sub_element.tag == 'display-name':
                                if sub_element.text.isdigit():
                                    ...
                                    channel_display_names.append(
                                        XMLTVDisplayName(
                                            language=sub_element.get('language'),
                                            text=sub_element.text,
                                        )
                                    )
                            elif sub_element.tag == 'icon':
                                channel_icons.append(
                                    XMLTVIcon(
                        ...
                        channel = XMLTVChannel(
                            provider=provider_map_class.constants_class().PROVIDER_NAME,
                        ...
                        db_session.add(
                            provider_map_class.channel_class()(
                        ...
                        db_session.add(
                            provider_map_class.program_class()(
    ...
    def _refresh_epg(cls, provider_name):
        logger.debug('%s EPG refresh timer triggered', provider_name)
        ...
        try:
            ...
            if cls._ignored_m3u8_groups_lock is not None:
                with cls._ignored_m3u8_groups_lock.reader_lock:
                    kwargs['ignored_m3u8_groups'] = copy.deepcopy(
                        cls._ignored_m3u8_groups
                    )
            if cls._m3u8_group_map_lock is not None:
                with cls._m3u8_group_map_lock.reader_lock:
                    kwargs['m3u8_group_map'] = copy.deepcopy(cls._m3u8_group_map)
    ...
    def _request_external_epg_xml(cls):
        ...
        url = '{0}'.format(
            Configuration.get_configuration_parameter(
                '{0}_EPG_URL'.format(
                    provider_map_class.constants_class().PROVIDER_NAME.upper()
                )
            )
        )
    ...
    def _start_refresh_epg_timer(cls, interval):
        if interval:
            logger.debug(
                'Started %s EPG refresh timer\nInterval => %s seconds',
                cls.__name__[:-3],
                interval,
            )

        cls._refresh_epg_timer = Timer(
            interval,
            cls._refresh_epg,
            [
                ProvidersController.get_provider_map_class(cls._provider_name)
                .database_access_class()
                .__name__
            ],
        )
    ...
    def generate_xmltv(
        ...
        yield '<?xml version="1.0" encoding="utf-8"?>\n<tv date="{0}" generator-info-name="IPTVProxy {1}">\n'.format(
            current_date_time_in_utc.strftime('%Y%m%d%H%M%S %z'), VERSION
        )
        ...
        with provider_map_class.database_class().get_access_lock().shared_lock:
            ...
            try:
                ...
                if style.capitalize() == EPGStyle.COMPLETE.value:
                    query_channels_xmltv = (
                        provider_map_class.database_access_class().query_channels_complete_xmltv
                    )
                else:
                    query_channels_xmltv = (
                        provider_map_class.database_access_class().query_channels_minimal_xmltv
                    )
                ...
                for channel_row in query_channels_xmltv(db_session):
                    yield channel_row.xmltv.format(
                        ...
                        server_hostname,
                        server_port,
                        '?http_token={0}'.format(server_password)
                ...
                cutoff_date_time_in_local = datetime.now(
                    tzlocal.get_localzone()
                ).replace(hour=0, minute=0, second=0, microsecond=0) + timedelta(
                    days=int(number_of_days) + 1
                )
                cutoff_date_time_in_utc = cutoff_date_time_in_local.astimezone(pytz.utc)

                if style.capitalize() == EPGStyle.COMPLETE.value:
                    query_programs_xmltv = (
                        provider_map_class.database_access_class().query_programs_complete_xmltv
                    )
                else:
                    query_programs_xmltv = (
                        provider_map_class.database_access_class().query_programs_minimal_xmltv
                    )

                for channel_row in query_programs_xmltv(
                    db_session, cutoff_date_time_in_utc
                ):
    ...
    def get_m3u8_groups(cls):
        ...
        with provider_map_class.database_class().get_access_lock().shared_lock:
            ...
            try:
                for (
                    channel_row
                ) in provider_map_class.database_access_class().query_channels_m3u8_groups(
                    db_session
                ):
...
class XStreamCodesProviderEPG(ProviderEPG):
    ...
    @classmethod
    def _do_ignore_channel(
        cls, channel, channel_group_map, ignored_channels, ignored_m3u8_groups
    ):
    ...
    def _parse_categories_json(cls):
        ...
        categories_json_stream = cls._request_epg_json(
            'categories_{0}.json'.format(username), 'get_live_categories'
        )

        categories_map = {}

        logger.debug(
            'Processing %s categories\nFile name => categories_%s.json',
            provider_map_class.constants_class().PROVIDER_NAME,
            username,
        )
        ...
        try:
            ...
            logger.debug(
                'Processed %s categories\nFile name => categories_%s.json',
                provider_map_class.constants_class().PROVIDER_NAME,
                username,
            )
        except Exception:
            logger.error(
                'Failed to process %s categories\nFile name => categories_%s.json',
                provider_map_class.constants_class().PROVIDER_NAME,
                username,
            )
    ...
    def _parse_channels_json(
        ...
        live_streams_stream = cls._request_epg_json(
            'channels_{0}.json'.format(username), 'get_live_streams'
        )

        logger.debug(
            'Processing %s channels\nFile name => channels_%s.json',
            provider_map_class.constants_class().PROVIDER_NAME,
            username,
        )
        ...
        try:
            ...
            for (prefix, event, value) in ijson_parser:
                if (prefix, event) == ('item.name', 'string'):
                    ...
                    if channel_m3u8_group is None:
                        channel_m3u8_group = '{0} - UNKNOWN'.format(
                    ...
                    channel = XMLTVChannel(
                        ...
                        if channel_icon_source
                        else [],
                        urls=[],
                    )
                    if not cls._do_ignore_channel(
                        channel,
                        channel_group_map,
                        ignored_channels,
                        ignored_m3u8_groups,
                        ...
                        if channel_xmltv_id in parsed_channel_xmltv_id_to_channel:
                            parsed_channel_xmltv_id_to_channel[channel_xmltv_id].append(
                                channel
                            )
                        else:
                            parsed_channel_xmltv_id_to_channel[channel_xmltv_id] = [
                                channel
                            ]
            ...
            logger.debug(
                'Processed %s channels\nFile name => channels_%s.json',
                provider_map_class.constants_class().PROVIDER_NAME,
                username,
            )
        except Exception:
            logger.error(
                'Failed to process %s channels\nFile name => channels_%s.json',
                provider_map_class.constants_class().PROVIDER_NAME,
                username,
            )
    ...
    def _parse_epg_xml(cls, db_session, parsed_channel_xmltv_id_to_channel):
        ...
        logger.debug(
            'Processing %s XML EPG\nFile name => xmltv_%s.xml',
            provider_map_class.constants_class().PROVIDER_NAME,
            username,
        )
            ...
            for (event, element) in etree.iterparse(
                ...
                if event == 'end':
                    if element.tag == 'channel':
                        ...
                        try:
                            for channel in parsed_channel_xmltv_id_to_channel[
                                program_channel_xmltv_id
                            ]:
                                program = XMLTVProgram(
                                    provider=provider_map_class.constants_class().PROVIDER_NAME,
    ...
    def _parse_m3u8_playlist(
        ...
        logger.debug(
            'Processing %s m3u8 playlist\nFile name => tv_channels_%s.m3u',
            provider_map_class.constants_class().PROVIDER_NAME,
            username,
        )
        ...
        try:
            for (m3u8_playlist_line_number, m3u8_playlist_line) in enumerate(
                m3u8_playlist_stream
            ):
                m3u8_playlist_line = m3u8_playlist_line.decode()

                if m3u8_playlist_line_number:
                    if m3u8_playlist_line_number % 2:
                        m3u8_line_tokens = re.findall(
                            r'(?:"[^"]*"|[^\s"])+', m3u8_playlist_line
                        )

                        for m3u8_line_token in m3u8_line_tokens:
                            if (
                                channel_xmltv_id is None
                                and 'tvg-id="' in m3u8_line_token
                            ):
                            ...
                            if (
                                channel_icon_source is None
                                and m3u8_line_token.startswith('tvg-logo="http')
                            ):
                                channel_icon_source = m3u8_line_token[
                                    len('tvg-logo="') : -1
                                ]
                            ...
                            if (
                                channel_m3u8_group is None
                                and 'group-title="' in m3u8_line_token
                            ):
                                channel_m3u8_group = '{0} - {1}'.format(
                                    ProvidersController.get_provider_map_class(
                                        cls._provider_name
                                    ...
                                    .PROVIDER_NAME,
                                    m3u8_line_token[
                                        len('group-title="') : m3u8_line_token.rfind(
                                            '"'
                                        )
                                    ],
                                )
                        ...
                        if match is not None:
                            ...
                            if not cls._do_ignore_channel(
                                ...
                                if (
                                    channel_xmltv_id
                                    in parsed_channel_xmltv_id_to_channel
                                ):
                                    parsed_channel_xmltv_id_to_channel[
                                        channel_xmltv_id
                                    ].append(channel)
                                ...
                                if (
                                    number_of_objects_added_to_db_session
                                    and number_of_objects_added_to_db_session % 1000
                                    == 0
                                ):
                                    db_session.flush()
            ...
            logger.debug(
                'Processed %s m3u8 playlist\nFile name => tv_channels_%s.m3u',
                provider_map_class.constants_class().PROVIDER_NAME,
                username,
            )
        except Exception:
            logger.error(
                'Failed to process %s m3u8 playlist\nFile name => tv_channels_%s.m3u',
                provider_map_class.constants_class().PROVIDER_NAME,
                username,
            )
    ...
    def _request_m3u8_playlist(cls):
        ...
        logger.debug(
            'Downloading %s tv_channels_%s.m3u\n'
            'URL => %s\n'
            '  Parameters\n'
            '    username => %s\n'
            '    password => %s\n'
    ...
    def _update_epg(cls, **kwargs):
        with cls._lock:
            ...
            try:
                ...
                db_session.add(
                    provider_map_class.setting_class()(
