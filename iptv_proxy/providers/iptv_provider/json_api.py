...
class ProviderConfigurationJSONAPI(object):
    ...
    def create_get_request_response_content(cls, configuration):
        ...
        provider_configuration_schema = (
            provider_map_class.configuration_class().get_configuration_schema()
        )
        ...
        if 'service' in provider_configuration_schema['Provider']:
            if (
                ...
                provider_configuration[
                    '{0}_service'.format(cls._provider_name)
                ] = configuration[
                    '{0}_SERVICE'.format(configuration_parameter_name_prefix)
                ]
            else:
        ...
        if 'server' in provider_configuration_schema['Provider']:
            if (
                ...
                provider_configuration[
                    '{0}_server'.format(cls._provider_name)
                ] = configuration[
                    '{0}_SERVER'.format(configuration_parameter_name_prefix)
                ]
            else:
        ...
        if 'url' in provider_configuration_schema['Provider']:
            if '{0}_URL'.format(configuration_parameter_name_prefix) in configuration:
                provider_configuration[
                    '{0}_url'.format(cls._provider_name)
        ...
        if 'username' in provider_configuration_schema['Provider']:
            if (
                ...
                provider_configuration[
                    '{0}_username'.format(cls._provider_name)
                ] = configuration[
                    '{0}_USERNAME'.format(configuration_parameter_name_prefix)
                ]
            else:
        ...
        if 'password' in provider_configuration_schema['Provider']:
            if (
                ...
                provider_configuration[
                    '{0}_password'.format(cls._provider_name)
                ] = configuration[
                    '{0}_PASSWORD'.format(configuration_parameter_name_prefix)
                ]
            else:
        ...
        if 'Playlist' in provider_configuration_schema:
            if 'protocol' in provider_configuration_schema['Playlist']:
                if (
                    ...
                    provider_configuration[
                        '{0}_playlist_protocol'.format(cls._provider_name)
                    ] = configuration[
            ...
            if 'type' in provider_configuration_schema['Playlist']:
                if (
                    ...
                    provider_configuration[
                        '{0}_playlist_type'.format(cls._provider_name)
                    ] = configuration[
                        '{0}_PLAYLIST_TYPE'.format(configuration_parameter_name_prefix)
                    ]
                else:
        ...
        if 'EPG' in provider_configuration_schema:
            if 'source' in provider_configuration_schema['EPG']:
                if (
                    ...
                    provider_configuration[
                        '{0}_epg_source'.format(cls._provider_name)
                    ] = configuration[
                        '{0}_EPG_SOURCE'.format(configuration_parameter_name_prefix)
                    ]
                else:
            ...
            if 'url' in provider_configuration_schema['EPG']:
                if (
                    ...
                    provider_configuration[
                        '{0}_epg_url'.format(cls._provider_name)
                    ] = configuration[
                        '{0}_EPG_URL'.format(configuration_parameter_name_prefix)
                    ]
                else:
    ...
    def create_validate_patch_request_body_schema(cls):
        ...
        validate_patch_request_body_schema[
            '{0}_enabled'.format(cls._provider_name)
        ] = required
