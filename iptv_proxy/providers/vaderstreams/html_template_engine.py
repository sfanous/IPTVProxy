...
class VaderStreamsHTMLTemplateEngine(ProviderHTMLTemplateEngine):
    ...
    def render_configuration_template(cls, environment, configuration, active_providers_map_class):
        ...
        configuration_vaderstreams_template = environment.get_template('configuration_vaderstreams.html')
        ...
        if 'VADERSTREAMS_SERVER' in configuration:
            configuration_vaderstreams_template_fields['configuration_{0}_selected'.format(
                configuration['VADERSTREAMS_SERVER'].lower().replace('-', '_'))] = 'selected="selected" '
        if 'VADERSTREAMS_USERNAME' in configuration:
            configuration_vaderstreams_template_fields['configuration_vaderstreams_username'] = configuration[
                'VADERSTREAMS_USERNAME']
        if 'VADERSTREAMS_PASSWORD' in configuration:
            configuration_vaderstreams_template_fields['configuration_vaderstreams_password'] = configuration[
        ...
        if 'VADERSTREAMS_PLAYLIST_PROTOCOL' in configuration:
            configuration_vaderstreams_template_fields[
                'configuration_vaderstreams_playlist_protocol_{0}_selected'.format(
                    configuration['VADERSTREAMS_PLAYLIST_PROTOCOL'].lower())] = 'selected="selected" '
        if 'VADERSTREAMS_PLAYLIST_TYPE' in configuration:
            configuration_vaderstreams_template_fields['configuration_vaderstreams_playlist_type_{0}_selected'.format(
                configuration['VADERSTREAMS_PLAYLIST_TYPE'].lower())] = 'selected="selected" '
        if 'VADERSTREAMS_EPG_SOURCE' in configuration:
            configuration_vaderstreams_template_fields['configuration_vaderstreams_epg_source_{0}_selected'.format(
                configuration['VADERSTREAMS_EPG_SOURCE'].lower())] = 'selected="selected" '
        if 'VADERSTREAMS_EPG_URL' in configuration:
            if configuration['VADERSTREAMS_EPG_URL'] is None:
                configuration_vaderstreams_template_fields['configuration_vaderstreams_epg_url'] = ''
            else:
                configuration_vaderstreams_template_fields['configuration_vaderstreams_epg_url'] = configuration[
                    'VADERSTREAMS_EPG_URL']

        return {provider_map_class.constants_class().PROVIDER_NAME: configuration_vaderstreams_template.render(
            configuration_vaderstreams_template_fields)}
