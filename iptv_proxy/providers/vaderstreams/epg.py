...
class VaderStreamsEPG(ProviderEPG):
    ...
    def _parse_categories_json(cls):
        ...
        categories_json_stream = cls._request_epg_json(
            VaderStreamsConstants.CATEGORIES_PATH,
            VaderStreamsConstants.CATEGORIES_JSON_FILE_NAME,
            {},
        )

        logger.debug(
            'Processing VaderStreams JSON categories\nFile name => %s',
            VaderStreamsConstants.CATEGORIES_JSON_FILE_NAME,
        )
        ...
        try:
            ...
            logger.debug(
                'Processed VaderStreams JSON categories\nFile name => %s',
                VaderStreamsConstants.CATEGORIES_JSON_FILE_NAME,
            )
        except Exception:
            logger.error(
                'Failed to process VaderStreams JSON categories\nFile name => %s',
                VaderStreamsConstants.CATEGORIES_JSON_FILE_NAME,
            )
    ...
    def _parse_channels_json(
        ...
        for category_id in categories_map:
            channels_json_stream = cls._request_epg_json(
                VaderStreamsConstants.CHANNELS_PATH,
                VaderStreamsConstants.CHANNELS_JSON_FILE_NAME,
                dict(category_id=category_id),
            )

            logger.debug(
                'Processing VaderStreams JSON channels\n'
                'File name => %s\n'
                'Category  => %s',
                VaderStreamsConstants.CHANNELS_JSON_FILE_NAME,
            ...
            try:
                        ...
                        try:
                            if channel_number not in parsed_channel_number_to_channel:
                                if 'sport' in channel_name.lower():
                                    ...
                                    channel_m3u8_group = 'VaderStreams - {0}'.format(
                                        categories_map[category_id]
                                    )
                                ...
                                channel = XMLTVChannel(
                                    ...
                                    display_names=[
                                        XMLTVDisplayName(
                                            language=None, text=channel_name
                                        )
                                    ],
                                    icons=[
                                        XMLTVIcon(
                ...
                logger.debug(
                    'Processed VaderStreams JSON channels\n'
                    'File name => %s\n'
                    'Category  => %s',
                    VaderStreamsConstants.CHANNELS_JSON_FILE_NAME,
                    categories_map[category_id],
                ...
                logger.error(
                    'Failed to process VaderStreams JSON channels\n'
                    'File name => %s\n'
                    'Category  => %s',
                    VaderStreamsConstants.CHANNELS_JSON_FILE_NAME,
                    categories_map[category_id],
    ...
    def _parse_epg_json(
        ...
        cls._parse_matchcenter_schedule_json(
            db_session, parsed_channel_number_to_channel
        )

        logger.debug(
            'Processed VaderStreams JSON EPG\nFile names => %s, %s, & %s',
            VaderStreamsConstants.CATEGORIES_JSON_FILE_NAME,
            VaderStreamsConstants.CHANNELS_JSON_FILE_NAME,
    ...
    def _parse_epg_xml(cls, db_session, parsed_channel_xmltv_id_to_channel):
        ...
        logger.debug(
            'Processing VaderStreams XML EPG\nFile name => %s',
            VaderStreamsConstants.XML_EPG_FILE_NAME,
        )
        ...
        with GzipFile(fileobj=epg_xml_stream) as input_file:
            ...
            try:
                ...
                logger.debug(
                    'Processed VaderStreams XML EPG\nFile name => %s',
                    VaderStreamsConstants.XML_EPG_FILE_NAME,
                )
            except Exception:
                logger.error(
                    'Failed to process VaderStreams XML EPG\nFile name => %s',
                    VaderStreamsConstants.XML_EPG_FILE_NAME,
                )
    ...
    @classmethod
    def _parse_matchcenter_schedule_json(
        cls, db_session, parsed_channel_number_to_channel
    ):
        ...
        matchcenter_schedule_json_stream = cls._request_epg_json(
            ...
            VaderStreamsConstants.MATCHCENTER_SCHEDULE_JSON_FILE_NAME,
            dict(
                start=int(current_date_time_in_utc.timestamp()),
                end=int(current_date_time_in_utc.timestamp()) + 172800,
            ),
        )

        logger.debug(
            'Processing VaderStreams JSON matchcenter schedule\nFile name => %s',
            VaderStreamsConstants.MATCHCENTER_SCHEDULE_JSON_FILE_NAME,
        )
        ...
        try:
            ...
            for (prefix, event, value) in ijson_parser:
                if (prefix, event) == ('item.category.name', 'string'):
                    program_categories.append(
                        XMLTVCategory(language=None, text=html.unescape(value))
                    )
                elif (prefix, event) == ('item.description', 'string'):
                    program_descriptions.append(
                        XMLTVDescription(language=None, text=html.unescape(value))
                    )
                    ...
                    program_start = datetime.strptime(
                        value[:-3] + value[-2:], '%Y-%m-%dT%H:%M:%S%z'
                    ).astimezone(pytz.utc)
                    ...
                    program_titles.append(
                        XMLTVTitle(language=None, text=html.unescape(value))
                    )
                elif (prefix, event) == ('item', 'end_map'):
            ...
            logger.debug(
                'Processed VaderStreams JSON matchcenter schedule\nFile name => %s',
                VaderStreamsConstants.MATCHCENTER_SCHEDULE_JSON_FILE_NAME,
            )
        except Exception:
            logger.error(
                'Failed to process VaderStreams JSON matchcenter schedule\n'
                'File name => %s',
                VaderStreamsConstants.MATCHCENTER_SCHEDULE_JSON_FILE_NAME,
            )
    ...
    def _request_epg_json(cls, epg_json_path, epg_json_file_name, request_parameters):
        ...
        response = Utility.make_http_request(
            ...
            url,
            params={'username': username, 'password': password, **request_parameters},
    ...
    def _request_epg_xml(cls):
        url = '{0}{1}'.format(
            VaderStreamsConstants.EPG_BASE_URL, VaderStreamsConstants.XML_EPG_FILE_NAME
        )

        logger.debug(
            'Downloading %s\nURL => %s', VaderStreamsConstants.XML_EPG_FILE_NAME, url
        )
    ...
    def _update_epg(cls, **kwargs):
        with cls._lock:
            ...
            try:
                if (
                    Configuration.get_configuration_parameter('VADERSTREAMS_EPG_SOURCE')
