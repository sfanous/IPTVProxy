...
class SecurityManager(object):
    ...
    def _determine_password_state(cls, password):
        ...
        try:
            ...
            if base64_decoded_encrypted_fernet_token[0] == 0x80:
                length_of_base64_decoded_encrypted_fernet_token = len(
                    base64_decoded_encrypted_fernet_token
                )

                if (
                    length_of_base64_decoded_encrypted_fernet_token < 73
                    or (length_of_base64_decoded_encrypted_fernet_token - 57) % 16 != 0
                ):
    ...
    def generate_self_signed_certificate(cls):
        ...
        if ip_address_location is not None:
            ...
            subject = issuer = x509.Name(
                [
                    x509.NameAttribute(
                        NameOID.COUNTRY_NAME, ip_address_location['countryCode']
                    ),
                    x509.NameAttribute(
                        NameOID.STATE_OR_PROVINCE_NAME, ip_address_location['region']
                    ),
                    x509.NameAttribute(
                        NameOID.LOCALITY_NAME, ip_address_location['city']
                    ),
            ...
            certificate = (
                ...
                .not_valid_before(current_date_time_in_utc)
                .not_valid_after(current_date_time_in_utc + timedelta(days=10 * 365))
                .add_extension(
        ...
        if cls._determine_password_state(password) == PasswordState.DECRYPTED:
            if cls._fernet is None:
                ...
                with Database.get_write_lock():
                    ...
                    try:
                        db_session.add(
                            Setting('password_encryption_key', fernet_key.decode())
                        )
                        db_session.commit()
                    except Exception:
            ...
            if cls._fernet:
                try:
                    ...
                    logger.debug(
                        'Decryption key loaded is valid for the encrypted %s password',
                        provider_name,
                    )
                except InvalidToken:
                    logger.error(
                        'Decryption key loaded is not valid for the encrypted %s password\n'
                ...
                logger.error(
                    '%s password is encrypted, but no decryption key was found\n'
                    'Please re-enter your cleartext password in the configuration file\n'
                    'Configuration file path => %s\n'
                    'Exiting...',
                    provider_name,
    ...
    @classmethod
    def set_auto_generate_self_signed_certificate(
        cls, auto_generate_self_signed_certificate
    ):
        cls._auto_generate_self_signed_certificate = (
            auto_generate_self_signed_certificate
        )
