...
class HLSClient(object):
    ...
    def __init__(self, id_, provider_name, channel_number):
        ...
        self._provider_map_class = ProvidersController.get_active_provider_map_class(provider_name)
    ...
    def download_chunks_m3u8(self, chunks_url):
        for number_of_download_attempts in range(1, 11):
            try:
                ...
                return self._provider_map_class.api_class().download_chunks_m3u8('127.0.0.1',
                                                                                 self._id,
                                                                                 chunks_url_components.path,
                                                                                 chunks_query_string_parameters)
            except requests.exceptions.HTTPError:
                time_to_sleep_before_next_attempt = math.ceil(number_of_download_attempts / 2) * 2

                logger.error(
                    'Attempt #{0}\n'
                    'Failed to download chunks.m3u8\n'
                    'Will try again in {1} seconds'.format(number_of_download_attempts,
        ...
        for number_of_download_attempts in range(1, 11):
            try:
                return self._provider_map_class.api_class().download_playlist_m3u8(
                    ...
                    dict(channel_number=self._channel_number,
                         protocol='hls'))
            except requests.exceptions.HTTPError:
                time_to_sleep_before_next_attempt = math.ceil(number_of_download_attempts / 5) * 5
    ...
    def download_ts_file(self, segment_url):
        ...
        return self._provider_map_class.api_class().download_ts_file('127.0.0.1',
                                                                     self._id,
