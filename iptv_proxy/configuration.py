...
class Configuration(object):
    ...
    def process_configuration_file_updates(cls):
        with cls._lock.writer_lock:
            ...
            if (
                loopback_hostname_updated
                or private_hostname_updated
                or public_hostname_updated
            ):
            ...
            for provider_name in sorted(ProvidersController.get_providers_map_class()):
                ProvidersController.get_provider_map_class(
                    provider_name
                ).configuration_class().process_configuration_file_updates(
        ...
        with cls._lock.writer_lock:
            ...
            try:
                ...
                configuration_object_md5 = hashlib.md5(
                    '{0}'.format(configuration_object).encode()
                ).hexdigest()
                ...
                try:
                    ...
                    try:
                        ...
                        try:
                            ...
                            if not Utility.is_valid_loopback_hostname(
                                ...
                                error_message_to_log.append(
                                    'The loopback option in the [Hostnames] section has an '
                        ...
                        try:
                            ...
                            if not Utility.is_valid_private_hostname(hostname_private):
                                if Utility.is_valid_public_hostname(hostname_private):
                                    error_message_to_log.append(
                                        'The private option in the [Hostnames] section has a '
                                        'public IP address\n'
                                    )
                                else:
                    ...
                    if do_determine_private_ip_address:
                        ...
                        if hostname_private:
                            error_message_to_log.append(
                                'The private option in the [Hostnames] section has an invalid '
                                'private IP address\n'
                    ...
                    if do_determine_public_ip_address:
                        ...
                        if hostname_public:
                            error_message_to_log.append(
                                'The public option in the [Hostnames] section has an invalid '
                                'public IP address\n'
                    ...
                    try:
                        ...
                        try:
                            ...
                            if not Utility.is_valid_port_number(http_port):
                                ...
                                error_message_to_log.append(
                                    'The http option in the [Ports] section must be a number '
                                    'between 0 and 65535\n'
                                )
                        except KeyError:
                            non_defaultable_error = True
                            ...
                            error_message_to_log.append(
                                ...
                                'The http option in the [Ports] section must be a number between '
                                '0 and 65535\n'
                            )
                        ...
                        try:
                            ...
                            if not Utility.is_valid_port_number(https_port):
                                ...
                                error_message_to_log.append(
                                    'The https option in the [Ports] section must be a number '
                                    'between 0 and 65535\n'
                                )
                        except KeyError:
                            non_defaultable_error = True
                            ...
                            error_message_to_log.append(
                                ...
                                'The https option in the [Ports] section must be a number between '
                                '0 and 65535\n'
                            )
                    except KeyError:
                        non_defaultable_error = True
                        ...
                        error_message_to_log.append(
                            'Could not find a [Ports] section in the '
                            '[Server] section\n'
                        )
                except KeyError:
                    non_defaultable_error = True
                ...
                for provider_name in sorted(
                    ...
                    ProvidersController.get_provider_map_class(
                        provider_name
                    ).configuration_class().read_configuration_file(
    ...
    def validate_update_configuration_request(cls, configuration):
        ...
        if not Utility.is_valid_loopback_hostname(
            ...
            errors['serverHostnameLoopback'] = (
                'Must be a valid loopback IP address or hostname\n'
        ...
        if not Utility.is_valid_private_hostname(
            ...
            if not Utility.is_valid_public_hostname(
                ...
                ] = 'Must be a valid private IP address, public IP address, or hostname'
        ...
        if not Utility.is_valid_public_hostname(
            ...
            ] = 'Must be a valid public IP address or hostname'
        ...
        for provider_name in sorted(ProvidersController.get_providers_map_class()):
            ProvidersController.get_provider_map_class(
                provider_name
            ).configuration_class().validate_update_configuration_request(
    ...
    def on_modified(self, event):
        with self._lock:
            if self._do_process_on_modified_event(event):
                logger.debug(
                    'Detected changes in configuration file\n'
                    'Configuration file path => %s',
                    self._file_path,
                )
    ...
    def process_optional_settings_file_updates(cls):
        with cls._lock.writer_lock:
            ...
            if (
                ...
                not in cls._previous_optional_settings
            ):
                cls._previous_optional_settings[
                    'lan_connections_require_credentials'
                ] = False
            ...
            for provider_name in sorted(ProvidersController.get_providers_map_class()):
                ProvidersController.get_provider_map_class(
                    provider_name
                ).optional_settings_class().process_optional_settings_file_updates(
                    cls._optional_settings, cls._previous_optional_settings
                )
        ...
        with cls._lock.writer_lock:
            ...
            try:
                ...
                cls._set_optional_settings(
                    json.loads(
                        optional_settings_file_content, object_pairs_hook=OrderedDict
                    )
                )
            except OSError:
                logger.error(
                    'Failed to read optional settings file\n'
                    'Optional settings file path => %s',
                    cls._optional_settings_file_path,
                )
            except JSONDecodeError:
                logger.error(
                    'Invalid optional settings file syntax\n'
                    'Optional settings file path => %s',
                    cls._optional_settings_file_path,
                )
...
class OptionalSettingsEventHandler(FileSystemEventHandler):
    ...
    def on_modified(self, event):
        with self._lock:
            if self._do_process_on_modified_event(event):
                logger.debug(
                    'Detected changes in optional settings file\n'
                    'Optional settings file path => %s',
                    self._file_path,
                )
